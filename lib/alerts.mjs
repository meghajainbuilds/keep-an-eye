...
export async function sendEmail({ to, from, key, item, price }) {
  const money = new Intl.NumberFormat('en-US', { style: 'currency', currency: item.currency }).format(price);
  const response = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' },
    body: JSON.stringify({ from, to: [to], subject: `Price alert: ${item.title.slice(0, 90)}`,
      text: `${item.title}\n\nThe store page now shows ${money}, at or below your target. Confirm the price, size, and availability at the store before purchasing.\n\n${item.url}` })
  });
  ...
  for (const item of store.list().filter(i => i.target_price != null || i.discount_percent != null)) {
    try {
      ...
      if (shouldAlert({ ...observed, last_notified_at: reset ? null : item.last_notified_at }, data.price, data.currency)) {
        if (!options.email?.to || !options.email?.key || !options.email?.from) {
          result.errors.push(`${item.id}: email is not configured`); continue;
        }
        await send({ ...options.email, item, price: data.price });
