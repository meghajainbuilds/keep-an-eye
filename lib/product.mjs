...
export function parseProduct(html, link, options = {}) {
  ...
  for (const product of products) {
    ...
    for (const offer of entries) {
      ...
      candidates.push({ product, offer, id,
        label: [product.color || offer?.itemOffered?.color, product.size || offer?.itemOffered?.size || offer?.name].filter(Boolean).join(' · ') || offer?.name || product.name || 'Product',
  ...
  if (!matches.length && !options.variant_id && unique.length === 1) {
    ...
    if (!requested.search && (!candidate.url || offered && offered.origin === requested.origin && offered.pathname === requested.pathname) || offered && urlKey(offered.href, link) === target) matches = [candidate];
  }
  ...
  const status = ambiguous ? (selectable.length ? 'needs_variant' : 'unavailable') : outOfStock ? 'out_of_stock' : price !== null ? 'verified' : 'unavailable';
  return {
    ...
    variants: [...new Map(selectable.map(c => [c.id, { id: c.id, label: c.label, url: c.url && urlKey(c.url, link) && new URL(urlKey(c.url, link)).origin === new URL(link).origin ? urlKey(c.url, link) : null }])).values()].slice(0, 100)
  };
}
