...
export function parseProduct(html, link, options = {}) {
  ...
  return {
    ...
    variants: [...new Map(unique.filter(c => c.id).map(c => [c.id, { id: c.id, label: c.label, url: c.url && urlKey(c.url, link) && new URL(urlKey(c.url, link)).origin === new URL(link).origin ? urlKey(c.url, link) : null }])).values()].slice(0, 100)
  };
}
